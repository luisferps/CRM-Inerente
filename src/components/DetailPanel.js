...
import { ETAPAS_FUNIL, ETAPAS_FUNIL_COMPLETO, ETAPAS_LABEL } from '../constants';
...
export default function DetailPanel({ cliente, onEdit, onDelete, onToggleFunil, onClose, onNovaNegociacao }) {
  ...
  useEffect(() => {
    ...
    // Carrega gerentes e corretores
    supabase.from('perfis').select('id, nome, role').eq('aprovado', true).order('nome')
  ...
  return (
    <div className="detail-panel">
      ...
      {/* Funil — só para Compra e Locação */}
      ...
      <div className="panel-section">
        <div className="panel-section-title">
          Histórico de Negociações {historico.length > 0 && `(${historico.length})`}
        </div>
        {loadingHist ? (
          <div style={{ fontSize: 12, color: '#9ca3af' }}>Carregando...</div>
        ) : historico.length === 0 ? (
          <div style={{ fontSize: 12, color: '#9ca3af' }}>Nenhuma negociação anterior.</div>
        ) : (
          historico.map(neg => (
            <div key={neg.id} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 7, padding: '10px 12px', marginBottom: 8, fontSize: 12 }}>
              ...
              {neg.corretor_original && neg.corretor_original !== neg.corretor && (
                <div style={{ color: '#9ca3af', fontSize: 11 }}>Original: {neg.corretor_original}</div>
              )}
      ...
      {transferModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 24, maxWidth: 380, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>🔁 Transferir Negociação</div>
            <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>
              Cliente: <strong>{cliente.nome}</strong><br />
              Corretor atual: <strong>{cliente.corretor || '—'}</strong>
              {cliente.corretor_original && cliente.corretor_original !== cliente.corretor && (
                <><br />Corretor original: <strong>{cliente.corretor_original}</strong></>
              )}
            </div>
            ...
            <select value={novoCorretor} onChange={e => setNovoCorretor(e.target.value)}
              ...
              {corretores.map(c => (
                <option key={c.id} value={c.id}>{c.nome} {c.role === 'gerente' ? '(Gerente)' : ''}</option>
              ))}
            </select>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => { setTransferModal(false); setNovoCorretor(''); }}
                style={{ padding: '8px 16px', borderRadius: 7, border: '1px solid #d1d5db', background: '#fff', color: '#6b7280', cursor: 'pointer' }}>
                Cancelar
              </button>
