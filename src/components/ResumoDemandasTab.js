...
// ── CatCard (componente separado para evitar hook dentro de map) ───────────────
function CatCard({ cat, agenda, onAtualizarSlot, onToggleDia, grupos }) {
  const [aberto, setAberto] = useState(false);
  const data = agenda.cats?.[cat.id] || {
    slots: [
      {time:'08:00',msg:'',days:[1,2,3,4,5]},
      {time:'13:00',msg:'',days:[1,2,3,4,5]},
      {time:'18:00',msg:'',days:[1,2,3,4,5]}
    ]
  };
  const grpCount = grupos.filter(g => g.cat === cat.id).length;

  return (
    <div style={{ background: '#111a15', border: '1px solid #1e2e22', borderRadius: 10, marginBottom: 10, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', cursor: 'pointer', userSelect: 'none' }} onClick={() => setAberto(a => !a)}>
      ...
      {aberto && (
        <div style={{ padding: '0 12px 12px', borderTop: '1px solid #1e2e22' }}>
          {data.slots.map((slot, i) => (
            <div key={i}>
              <div style={{ fontSize: 9, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', color: '#3d5242', margin: '10px 0 4px' }}>Horário {i+1}</div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#0a0f0d', border: '1px solid #243329', borderRadius: 20, padding: '3px 8px', marginBottom: 6 }}>
                <span style={{ fontSize: 10, color: '#7a9980' }}>⏰</span>
                <input type="time" value={slot.time}
                  onChange={e => onAtualizarSlot(cat.id, i, 'time', e.target.value)}
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
                {DAYS.map((d, j) => {
                  const on = slot.days?.includes(j);
                  return (
                    <span key={j} onClick={() => onToggleDia(cat.id, i, j)}
                      style={{ padding: '2px 7px', borderRadius: 20, fontSize: 10, fontWeight: 500, cursor: 'pointer', border: `1px solid ${on ? '#1a9e4a' : '#243329'}`, background: on ? 'rgba(37,211,102,.12)' : 'transparent', color: on ? '#25d366' : '#3d5242', userSelect: 'none' }}>
                      {d}
                    </span>
                  );
                })}
              </div>
              <textarea value={slot.msg}
                onChange={e => onAtualizarSlot(cat.id, i, 'msg', e.target.value)}
                placeholder={`Mensagem horário ${i+1}...`}
                style={{ width: '100%', background: '#0a0f0d', border: '1px solid #243329', borderRadius: 8, padding: '8px 10px', fontFamily: "'Sora',sans-serif", fontSize: 11, color: '#e8f0ea', resize: 'none', outline: 'none', minHeight: 60, lineHeight: 1.6, marginBottom: 4, boxSizing: 'border-box' }}
                rows={3} />
            </div>
          ))}
        </div>
...
function WAScheduler({ instancia, textoParaDisparo, onTextoUsado }) {
  ...
  async function salvarAgenda(ag) {
    setSalvando(true);
    try {
      const r = await fetch(`${WA_AGENT_URL}/scheduler/update`, {
        ...
        body: JSON.stringify({ ...ag, instancia, categorias: CATS })
      });
      if (r.ok) toast('Salvo!');
  ...
  async function sincronizar() {
    try {
      ...
      if (res.ok) { await carregarAgenda(); toast('Grupos sincronizados!'); }
  ...
  function atualizarSlot(catId, slotIdx, campo, valor) {
    setAgenda(prev => {
      ...
      if (!novaCats[catId]) novaCats[catId] = { slots: [{time:'08:00',msg:'',days:[1,2,3,4,5]},{time:'13:00',msg:'',days:[1,2,3,4,5]},{time:'18:00',msg:'',days:[1,2,3,4,5]}] };
      const slots = [...novaCats[catId].slots];
      slots[slotIdx] = { ...slots[slotIdx], [campo]: valor };
  ...
  function toggleDia(catId, slotIdx, dayIdx) {
    setAgenda(prev => {
      const novaCats = { ...prev.cats };
      if (!novaCats[catId]) return prev;
      const slots = [...novaCats[catId].slots];
      const days = slots[slotIdx].days.includes(dayIdx)
        ? slots[slotIdx].days.filter(d => d !== dayIdx)
        : [...slots[slotIdx].days, dayIdx];
      slots[slotIdx] = { ...slots[slotIdx], days };
      novaCats[catId] = { ...novaCats[catId], slots };
      return { ...prev, cats: novaCats };
    });
  }
  ...
  function criarCategoria() {
    ...
    setCATS(prev => [...prev, { id, name: modalNomeCat.trim(), cor: modalCorCat }]);
  ...
  function excluirCategoria(id) {
    ...
    setAgenda(prev => {
      const novaCats = { ...prev.cats };
      delete novaCats[id];
      return { ...prev, cats: novaCats, grupos: prev.grupos.map(g => g.cat === id ? { ...g, cat: '' } : g) };
    });
    setModalDelCat(null);
  ...
  const gruposFiltrados = filtroGrupo
    ? (agenda.grupos || []).filter(g => g.name.toLowerCase().includes(filtroGrupo.toLowerCase()))
    : (agenda.grupos || []);

  const inputStyle = { width: '100%', background: '#0a0f0d', border: '1px solid #243329', borderRadius: 8, padding: '7px 10px', fontFamily: "'Sora',sans-serif", fontSize: 12, color: '#e8f0ea', outline: 'none', boxSizing: 'border-box', marginBottom: 4 };
  const btnSalvar = { width: '100%', padding: 10, borderRadius: 8, border: 'none', background: salvando ? '#3d5242' : '#25d366', color: '#000', fontFamily: "'Sora',sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer', marginTop: 8 };

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#0a0f0d', borderRadius: 12, overflow: 'hidden', fontFamily: "'Sora',sans-serif", color: '#e8f0ea', fontSize: 13, position: 'relative' }}>
      {/* Header */}
      <div style={{ padding: '10px 14px', borderBottom: '1px solid #1e2e22', display: 'flex', alignItems: 'center', gap: 8, background: '#111a15', flexShrink: 0 }}>
        <div style={{ width: 24, height: 24, background: '#25d366', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, flexShrink: 0 }}>💬</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 13 }}>WA Scheduler</div>
          <div style={{ fontSize: 10, color: '#7a9980' }}>{instancia}</div>
        </div>
      </div>

      {/* Nav */}
      <div style={{ display: 'flex', borderBottom: '1px solid #1e2e22', background: '#0a0f0d', flexShrink: 0 }}>
        {[['mensagens','📋 Msg'],['disparo','⚡ Disparo'],['grupos','👥 Grupos'],['categorias','🏷️ Cats'],['config','⚙️']].map(([id,label]) => (
          <button key={id} onClick={() => setSecAtiva(id)}
            style={{ flex: 1, minWidth: 50, padding: '9px 4px', background: 'transparent', border: 'none', fontFamily: "'Sora',sans-serif", fontSize: 9, fontWeight: 500, color: secAtiva===id ? '#25d366' : '#7a9980', cursor: 'pointer', borderBottom: `2px solid ${secAtiva===id ? '#25d366' : 'transparent'}`, whiteSpace: 'nowrap' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        {carregando ? (
          ...
          <>
            ...
            {secAtiva === 'mensagens' && (
              <>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', letterSpacing: '.5px', marginBottom: 10, textTransform: 'uppercase' }}>Mensagens programadas</div>
                {CATS.map(cat => (
                  <CatCard key={cat.id} cat={cat} agenda={agenda} grupos={agenda.grupos||[]}
                    onAtualizarSlot={atualizarSlot} onToggleDia={toggleDia} />
                ))}
                <button style={btnSalvar} disabled={salvando} onClick={() => salvarAgenda(agenda)}>
            ...
            {secAtiva === 'disparo' && (
              <>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', letterSpacing: '.5px', marginBottom: 8, textTransform: 'uppercase' }}>Disparo imediato</div>
                <textarea value={disparoMsg} onChange={e => setDisparoMsg(e.target.value)}
                  placeholder="Digite a mensagem..."
                  style={{ width: '100%', background: '#0a0f0d', border: '1px solid #243329', borderRadius: 8, padding: '8px 10px', fontFamily: "'Sora',sans-serif", fontSize: 11, color: '#e8f0ea', resize: 'none', outline: 'none', minHeight: 80, lineHeight: 1.6, boxSizing: 'border-box' }} />
                <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', letterSpacing: '.5px', margin: '10px 0 8px', textTransform: 'uppercase' }}>Categorias</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 8 }}>
                  {CATS.map(cat => {
                    const sel = catsSelDisparo.has(cat.id);
                    return (
                      <div key={cat.id} onClick={() => toggleCatDisparo(cat.id)}
                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', background: '#0a0f0d', border: `2px solid ${sel ? cat.cor : '#243329'}`, borderRadius: 8, cursor: 'pointer', userSelect: 'none' }}>
                        <div style={{ width: 7, height: 7, borderRadius: '50%', background: cat.cor, flexShrink: 0 }} />
                        <span style={{ fontSize: 11, fontWeight: 500, color: sel ? '#e8f0ea' : '#7a9980', flex: 1, lineHeight: 1.3 }}>{cat.name}</span>
                        {sel && <span style={{ color: cat.cor, fontSize: 12 }}>✓</span>}
                      </div>
                    );
                  })}
                </div>
                ...
                <button disabled={disparando} onClick={dispararAgora}
                  style={{ width: '100%', padding: 10, borderRadius: 8, border: 'none', background: disparando ? '#9ca3af' : '#ff4d4d', color: '#fff', fontFamily: "'Sora',sans-serif", fontSize: 12, fontWeight: 700, cursor: disparando ? 'not-allowed' : 'pointer', marginTop: 4 }}>
                  {disparando ? 'Enviando...' : '⚡ Enviar agora'}
                </button>
                {logDisparo.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', letterSpacing: '.5px', marginBottom: 6, textTransform: 'uppercase' }}>Resultado</div>
                    {logDisparo.map((l, i) => (
                      <div key={i} style={{ fontSize: 11, padding: '4px 8px', background: '#0a0f0d', borderRadius: 6, borderLeft: `3px solid ${l.ok ? '#25d366' : '#ff4d4d'}`, marginBottom: 4, color: l.ok ? '#e8f0ea' : '#7a9980' }}>
                        {l.ok ? '✓' : '✗'} {l.nome}{l.status ? ` (${l.status})` : ''}
                      </div>
                    ))}
                  </div>
                )}
            ...
            {secAtiva === 'grupos' && (
              <>
                <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                  {[{ n: (agenda.grupos||[]).length, label: 'Total', cor: '#25d366' }, { n: (agenda.grupos||[]).filter(g=>!g.cat).length, label: 'Sem cat.', cor: '#f0a500' }].map((s, i) => (
                    <div key={i} style={{ flex: 1, minWidth: 55, background: '#111a15', border: '1px solid #1e2e22', borderRadius: 8, padding: '6px 8px', textAlign: 'center' }}>
                      <div style={{ fontSize: 16, fontWeight: 700, color: s.cor }}>{s.n}</div>
                      <div style={{ fontSize: 9, color: '#7a9980' }}>{s.label}</div>
                    </div>
                  ))}
                  {CATS.map(cat => {
                    const n = (agenda.grupos||[]).filter(g=>g.cat===cat.id).length;
                    return (
                      <div key={cat.id} style={{ flex: 1, minWidth: 55, background: '#111a15', border: '1px solid #1e2e22', borderRadius: 8, padding: '6px 8px', textAlign: 'center' }}>
                        <div style={{ fontSize: 16, fontWeight: 700, color: cat.cor }}>{n}</div>
                        <div style={{ fontSize: 9, color: '#7a9980' }}>{cat.name.split(' ')[0]}</div>
                      </div>
                    );
                  })}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, padding: '6px 10px', background: '#111a15', border: '1px solid #1e2e22', borderRadius: 8 }}>
                  <span style={{ fontSize: 11, color: '#7a9980' }}>Sync automático</span>
                ...
                <input value={filtroGrupo} onChange={e => setFiltroGrupo(e.target.value)}
                  placeholder="🔍 Buscar grupo..." style={{ ...inputStyle, marginBottom: 8 }} />
                {gruposFiltrados.map(g => (
                  <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', background: '#111a15', border: '1px solid #1e2e22', borderRadius: 8, marginBottom: 6 }}>
            ...
            {secAtiva === 'categorias' && (
              <>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', letterSpacing: '.5px', marginBottom: 10, textTransform: 'uppercase' }}>Gerenciar categorias</div>
                {CATS.map(cat => (
                  <div key={cat.id} style={{ background: '#111a15', border: '1px solid #1e2e22', borderRadius: 10, marginBottom: 8, padding: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <button onClick={() => setModalDelCat(cat)}
                        style={{ background: 'transparent', border: '1px solid #243329', borderRadius: 6, padding: '4px 8px', fontSize: 12, color: '#7a9980', cursor: 'pointer' }}>🗑</button>
                    </div>
            ...
            {secAtiva === 'config' && (
              <>
                <div style={{ background: '#111a15', border: '1px solid #1e2e22', borderRadius: 10, padding: 12, marginBottom: 14 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 8 }}>Instância ativa</div>
                  ...
                  <button onClick={() => window.open('https://evolution-api-production-6f9a.up.railway.app/manager', '_blank')}
                    style={{ width: '100%', padding: 7, borderRadius: 7, border: '1px solid #243329', background: 'transparent', color: '#7a9980', fontFamily: "'Sora',sans-serif", fontSize: 11, cursor: 'pointer' }}>
                    Abrir Evolution Manager →
                  </button>
                </div>
                <div style={{ background: '#111a15', border: '1px solid #1e2e22', borderRadius: 10, padding: 12 }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: '#7a9980', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 8 }}>Usar outra conta</div>
      ...
      {/* Toast */}
      {toastMsg && (
        <div style={{ position: 'absolute', top: 8, left: '50%', transform: 'translateX(-50%)', background: '#111a15', border: `1px solid ${toastWarn ? '#f0a500' : '#1a9e4a'}`, borderRadius: 10, padding: '7px 14px', fontSize: 12, fontWeight: 500, color: toastWarn ? '#f0a500' : '#25d366', whiteSpace: 'nowrap', zIndex: 10 }}>
          {toastMsg}
        </div>
      )}
          ...
          <div style={{ background: '#111a15', border: '1px solid #1e2e22', borderRadius: 14, padding: 20, width: '100%', maxWidth: 280 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <input value={modalNomeCat} onChange={e => setModalNomeCat(e.target.value)} placeholder="Nome da categoria"
                style={{ ...inputStyle, margin: 0, flex: 1 }} autoFocus onKeyDown={e => e.key === 'Enter' && criarCategoria()} />
            </div>
      ...
      {modalDelCat && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20, padding: 16 }}>
          <div style={{ background: '#111a15', border: '1px solid #1e2e22', borderRadius: 14, padding: 20, width: '100%', maxWidth: 280 }}>
            ...
            <p style={{ fontSize: 11, color: '#f0a500', marginBottom: 16 }}>Os grupos ficarão sem categoria.</p>
            <div style={{ display: 'flex', gap: 8 }}>
      ...
      <div style={{ flex: '0 0 42%', position: 'sticky', top: 0, height: 'calc(100vh - 120px)', minHeight: 500 }}>
        {instancia ? (
          ...
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: 24, textAlign: 'center', color: textMuted }}>
            ...
            <div style={{ fontSize: 12 }}>Configure sua instância do WhatsApp no Perfil para usar o disparador aqui.</div>
          </div>
        )}
      </div>
    </div>
